...
const plantillaBase = (contenido) => `
  <div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;border:1px solid #e0e0e0;border-radius:8px;overflow:hidden">
    <div style="background:#1a3a6b;padding:20px;text-align:center">
      ...
      <h1 style="color:#FFD700;margin:0;font-size:20px">by Total Pacific Group</h1>
    </div>
...
const enviarCorreo = async ({ to, subject, html }) => {
  try {
    ...
    // Solo loguea, nunca lanza el error hacia arriba
    console.warn(`Correo no enviado a ${to} — se continuará sin bloquear:`, err.message);
  }
};

// Correo: nuevo ticket creado (va a todos los admins)
...
// Correo: cambio de estado (va al usuario dueño del ticket)
...
const notificarUsuarioCreado = async (usuario, passwordTemporal) => {
  await enviarCorreo({
    ...
    html: `
      ...
      <p style="color:#D32F2F">Por seguridad, solicita un cambio de tu contraseña al ingresar por primera vez.</p>
      <a href="${process.env.FRONTEND_URL}" style="background:#1a3a6b;color:#FFD700;padding:10px 20px;border-radius:4px;text-decoration:none;display:inline-block;margin-top:10px">Ingresar al sistema</a>
    `,
  });
};
